...
import { breezyCloseNonMember, breezyCoverPopup } from "~/images";
...
const NonMemberReminderModal = ({ isOpen, onClose }: ModalProps) => {
  return (
    <div>
      {/* Modal overlay */}
      {isOpen && (
      ...
      {isOpen && (
        <div className='!fixed max-md:left-0 z-50 transform overflow-hidden rounded-xl bg-white shadow-lg max-md:flex max-md:w-full max-md:items-center max-md:justify-center max-md:overflow-y-auto lg:left-1/2 top-1/2 lg:-translate-x-1/2 -translate-y-1/2'>
          <div className='flex w-full flex-col max-md:mx-2 h-[440px] lg:w-[545px] lg:flex-row'>
            ...
            <div className="flex flex-col lg:pl-10 lg:pr-5 pt-5 bg-[#FFFFFF]">
              <div className="flex justify-end" onClick={onClose}>
                <NextImage src={breezyCloseNonMember.src} width={34} height={34} alt="Close Membership" className="cursor-pointer" />
              </div>
              <p className="text-[#2A3B80] text-sm mt-6 pr-5 tracking-[4px] font-katide-medium">DRIZY VIP+</p>
              <h1 className="text-[#1A214C] text-4xl font-katide-heavy mt-6">This is Premium Feature</h1>
              <p className="text-[#2A3B80] text-sm mt-6 font-katide-regular pr-5">Want access to Drizy? Join any Drizy VIP+ plan and unlock all the amazing features. Click the button below to get started—it’s quick and easy!</p>
...
export default NonMemberReminderModal;