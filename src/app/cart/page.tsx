...
export default function Register() {
      ...
      try {
        ...
        if (activeSubcription.subcription !== undefined) {
          const payload: { [key: string]: string | number } = {
            ...
            licenseType: 2, // auto Business saat subscription aktif
          };
  ...
  const handleCheckout = () => {
    if (activeSubcription.subcription) {
      handleDownload();
    } else {
      handlePayment();
    }
  ...
  return (
    <main className='bg-[#ffffff]'>
      <section className='flex flex-col gap-4 max-md:p-2 lg:flex-row lg:py-20 mx-auto w-full max-w-[1164px]'>
        <div className='flex basis-3/12 flex-col items-center gap-12 rounded-xl bg-white p-8 pr-16 shadow-lg border'>
          <p className='text-3xl font-bold'>My Cart</p>
          <CartLottie />
        </div>
        <div className='flex h-1/2 flex-grow flex-col items-center justify-center gap-4 rounded-xl bg-white p-4 shadow-lg lg:p-8 border'>
          {cart.length === 0 ? (
            ...
            <>
              <table className='table-fixed'>
                ...
                <tr>
                  ...
                  <th></th>
                  <th>Product</th>
                  {!activeSubcription.subcription && <th>License Type</th>}
                  {!activeSubcription.subcription && <th>Price</th>}
                </tr>
                </thead>
                ...
                {cart.map((item) => (
                  <tr key={item.id}>
                    ...
                    {!activeSubcription.subcription && (
                      <td className='w-2/12'>
                        {
                          { 0: 'Personal', 1: 'Commercial', 2: 'Business' }[item.licenseType] ?? '-'
                        }
                      </td>
                    )}
                    {!activeSubcription.subcription && (
              ...
              >
                {activeSubcription.subcription ? 'Download' : 'Checkout'}
              </button>
            </>
          )}
        </div>
